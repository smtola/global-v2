...
const CareerAdmin = () => {
  ...
  const editId = async (id) => {
    ...
    const career = data.find((item) => item.id === id);
    const careerDefaul = fileDefault.find((item) => item.id === id);
    if (career) {
  ...
  const updateCareer = async (imageUrl = null) => {
    setUploading(true);
    try {
      const updatedData = {
        title,
