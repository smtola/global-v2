...
const BannerConfig = () => {
  ...
  const [descriptionKh, setDescKh] = useState('');
  ...
  const editId = async (id) =>{
    ...
    data.map((items) => {
      if (items.id === id) {
        setBannerId(items.id);
        setDescEn(items.descriptionEn);
        setDescKh(items.descriptionKh);
  ...
  const handleUpdate = async () => {
    if (!descriptionEn || !descriptionKh) {
